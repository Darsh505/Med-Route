...
export default function AdminPortalPage() {
  ...
  // Store editable hospital telemetry in local state
  const [hospitals, setHospitals] = useState<HospitalOption[]>(ALL_HOSPITALS);
  const [selectedHospitalId, setSelectedHospitalId] = useState<string>(ALL_HOSPITALS[0]?.id || "");
  ...
  const stats = useMemo(() => {
    ...
    return { totalFacilities, totalIcuBeds, availableIcuBeds, pmjayCount };
  }, [hospitals]);
  ...
  const updateIcuBeds = (delta: number) => {
    ...
    setHospitals((prev) =>
      prev.map((h) => (h.id === selectedHospital.id ? { ...h, beds_icu_available: newCount } : h))
    );
  ...
  const togglePmjayStatus = () => {
    ...
    setHospitals((prev) =>
      prev.map((h) => (h.id === selectedHospital.id ? { ...h, is_pmjay_empanelled: newStatus } : h))
    );
      ...
      <div className="min-h-screen bg-black text-zinc-100 flex flex-col items-center justify-center p-4">
        <div className="w-full max-w-sm bg-zinc-950 border border-zinc-800 rounded-lg p-6 shadow-2xl">
          ...
          <form onSubmit={handleLogin} className="flex flex-col gap-4">
            ...
            <button
              type="submit"
              className="w-full py-2 bg-white text-black hover:bg-zinc-200 text-xs font-medium rounded transition-colors"
            >
      ...
      <header className="h-14 border-b border-zinc-800 bg-zinc-950 px-6 flex items-center justify-between sticky top-0 z-40">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="text-xs text-zinc-400 hover:text-red-400 px-2.5 py-1 rounded border border-zinc-800 hover:border-zinc-700 transition-colors"
          >
            Lock
          </button>
        </div>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 flex flex-col gap-5">
        ...
        {notification && (
          <div className="py-2 px-3 bg-zinc-900 border border-zinc-800 text-zinc-200 text-xs font-mono rounded flex items-center justify-between">
            ...
            <button onClick={() => setNotification("")} className="text-zinc-500 hover:text-white text-xs">
              ×
            </button>
          </div>
        )}

        {/* Dynamic Metric Cards (Calculated across all 1,451+ facilities) */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <div className="bg-zinc-950 border border-zinc-850 rounded p-3.5">
            ...
            <span className="text-[10px] text-zinc-500 mt-0.5 block">State &amp; Private Beds</span>
          </div>
          ...
          <div className="bg-zinc-950 border border-zinc-850 rounded p-3.5">
            ...
            <span className="text-[10px] text-zinc-500 mt-0.5 block">Live Telemetry</span>
          </div>
          ...
          <div className="lg:col-span-7 bg-zinc-950 border border-zinc-850 rounded flex flex-col overflow-hidden">
            ...
            <div className="p-3 border-t border-zinc-850 flex items-center justify-between text-xs text-zinc-400 bg-zinc-950">
              <button
                ...
                className="px-3 py-1 bg-black border border-zinc-800 rounded disabled:opacity-30 disabled:cursor-not-allowed hover:bg-zinc-900 text-zinc-200 transition-colors"
              >
                Previous
              </button>
            ...
            {selectedHospital ? (
              <>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1.5 font-mono">
                      <button
                        ...
                        className="w-8 h-8 rounded bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-white font-bold flex items-center justify-center transition-colors"
                        title="Decrement bed count"
                      >
                        -
                      </button>
                      <button
                        onClick={() => updateIcuBeds(1)}
                        className="w-8 h-8 rounded bg-white hover:bg-zinc-200 text-black font-bold flex items-center justify-center transition-colors"
                        title="Increment bed count"
                      >
                        +
                      </button>
                    </div>
                ...
                <div className="flex flex-col gap-2 text-xs">
                  <div className="flex items-center justify-between p-2.5 rounded bg-black border border-zinc-850">
                    ...
                    <button
                      ...
                      className={`text-[11px] px-2.5 py-1 rounded font-mono font-medium transition-colors ${
                ...
                <button
                  ...
                  className="w-full py-2 bg-white text-black hover:bg-zinc-200 font-medium text-xs rounded transition-colors"
                >
          ...
          <div className="bg-zinc-950 border border-zinc-800 rounded-lg max-w-2xl w-full p-5 shadow-2xl flex flex-col max-h-[85vh]">
            <div className="flex items-center justify-between border-b border-zinc-850 pb-3">
              ...
              <button
                ...
                className="text-zinc-500 hover:text-white text-sm font-mono px-2 py-1"
              >
                Close
              </button>
            </div>
            ...
            <div className="border-t border-zinc-850 pt-3 flex justify-end">
              <button
                onClick={() => setShowTariffModal(false)}
                className="px-4 py-1.5 bg-white text-black hover:bg-zinc-200 text-xs font-medium rounded transition-colors"
              >
                Done
              </button>
            </div>
